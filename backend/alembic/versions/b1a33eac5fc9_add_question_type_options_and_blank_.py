...
def upgrade() -> None:
    ...
    questiontype_enum = postgresql.ENUM('FREE_FORM', 'MULTIPLE_CHOICE', 'YES_NO', 'FILL_IN_BLANK', name='questiontype', create_type=False)
    questiontype_enum.create(op.get_bind(), checkfirst=True)

    # Add columns with default for existing rows
    op.add_column('questions', sa.Column('question_type', sa.Enum('FREE_FORM', 'MULTIPLE_CHOICE', 'YES_NO', 'FILL_IN_BLANK', name='questiontype'), nullable=False, server_default='FREE_FORM'))
    op.add_column('questions', sa.Column('options', postgresql.ARRAY(sa.Text()), nullable=True))
